...
import { fetchTilesIndex, fetchNetworkData, fetchImagery, fetchMetadata } from './services/googleDriveService';
  ...
  for (let r = minRow; r <= maxRow; r++) {
    for (let c = minCol; c <= maxCol; c++) {
      ...
      if (tile) {
        ...
        const promise = (async () => {
          try {
            const imgDataUrl = await fetchImagery(tile.tile_id, year);
            ...
            return new Promise((resolve) => {
              ...
              img.onerror = () => {
                console.warn(`Failed to load image for ${tile.tile_id} (${year})`);
                resolve();
              };
              img.src = imgDataUrl;
            });
          } catch (error) {
            console.warn(`Failed to fetch image for ${tile.tile_id} (${year}):`, error);
          }
        })();
...
export default function App() {
  ...
  const [singleTileOpacity, setSingleTileOpacity] = useState(0.8);
        ...
        const tilesWithMetadata = await Promise.all(
          tiles.map(async (tile) => {
            try {
              ...
              const metadata = await fetchMetadata(tile.tile_id);
              return {
                ...tile,
              ...
              console.warn(`No metadata for ${tile.tile_id}, using bounds`);
  ...
  useEffect(() => {
    ...
    const tilePromises = tilesToLoad.map(tileID => {
      const yearPromises = YEARS.map(year => {
        return fetchNetworkData(tileID, year)
          .then(data => ({ year, data }))
          .catch(() => null);
      });
  ...
  // Load imagery from Google Drive when tile/year changes (SINGLE TILE ONLY)
  useEffect(() => {
    if (!activeTileID || Array.isArray(activeTileID)) {
    ...
    fetchImagery(activeTileID, currentYear)
      .then(url => {
        console.log(`✅ Loaded imagery for ${activeTileID} (${currentYear})`);
        setCurrentImageUrl(url);
      })
      .catch(error => {
        console.warn(`⚠️ Failed to load imagery for ${currentYear}:`, error);
        setCurrentImageUrl(null);
      });
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        {/* Go Back Button, Settings Panel, Statistics Panel, Timeline... */}
        {/* (REST OF JSX IDENTICAL TO YOUR ORIGINAL CODE - TOO LONG TO INCLUDE) */}
      </div>
    </div>
  );
}